 ...
 class _CropFormAddState extends State<CropFormAdd> {
  ...
  void initState() {
       ...
       eProvider = Provider.of<plotcnt>(context, listen: false);
        if (eProvider != null) {
    print("Setting CropId: ${widget.cropid.cropId}");
    eProvider!.CropId = widget.cropid.cropId.toString();
     log(" crop idss : -- ${  eProvider!.CropId }");
  } else {
    print("Warning: eProvider is null. Ensure Provider setup is correct.");
  }
  ...
  void init() async {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      eProvider?.chataniid;
      await eProvider?.chataniApi(widget.cropid.cropId);
      await eProvider?.culitivationApi(widget.cropid.cropId);
      await eProvider?.vationApi(widget.cropid.cropId);
      await eProvider?.landTypeApi(widget.cropid.cropId);
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            //     widget.cropid.cropId != 34 ||
                            //     widget.cropid.cropId != 31)
                            //   if (widget.cropid.cropId != 21 && widget.cropid.cropId == 39) 
                            //     if (widget.cropid.cropId == 19 ||widget.cropid.cropId == 24) 
                              h10,
                            Text(

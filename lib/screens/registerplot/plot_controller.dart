...
class plotcnt extends ChangeNotifier {
  ...
  Future culitivationApi(cropid) async {
  ...
  Future chataniApi(cropid) async {
    _setLoading(true);
    try {
      await ApiService.instance
          .postHTTP(
  ...
  Future landTypeApi(cropid) async {
    _setLoading(true);
    try {
      await ApiService.instance
          .postHTTP(
  ...
  Future rigstar(BuildContext context) async {
   ...
   validationRegtrationcrop(context) 
  {
     ...
     if (CropId == '1')
      ...
      if (veriteytype == null) 
      {
       ...
       if (datetimeContoller.text.isEmpty) {
        Utils().validationTostmassage(
    ...
    if (CropId == '22' || CropId == '33' || CropId == '17') {
      ...
      } else if ( CropId != "21" && landtype == null) {
        Utils().validationTostmassage(
          jaminprakar.tr,
        );
      } else if (plantingDistance1.text.isEmpty) {
   ...
   bool toastShown = false; // Declare a flag to track whether the toast has been shown

if (CropId == '9' && !toastShown) { // Check if CropId is '9' and toast hasn't been shown
  if (cultivationType == null) {
    Utils().validationTostmassage(txt_lagvadiche_prakar.tr);
  } else if (datetimeContoller.text.isEmpty) {
    Utils().validationTostmassage(txt_date_of_planting.tr);
  } else if (veriteytype == null) {
    Utils().validationTostmassage(txt_variety.tr);
  } else if (CropId != "22" && CropId != "21" && landtype == null) {
    Utils().validationTostmassage(jaminprakar.tr);
  } else if (waterScorceController.text.isEmpty) {
    Utils().validationTostmassage(select_irrigation_source.tr);
  } else if (selectedOption == ' ') {
    Utils().validationTostmassage(txt_lagvad_padhat.tr);
  } else if (plantingDistance.text.isEmpty) {
    Utils().validationTostmassage(txt_planting_distance.tr);
  } else if (plantingDistance1.text.isEmpty) {
    Utils().validationTostmassage(txt_planting_distance.tr);
